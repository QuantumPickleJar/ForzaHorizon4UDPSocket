...
class ForzaDataPacket:
    ...
    sled_format = '<iIffff fffffffffffffffffffffffffffffffffffffffffffffffiiiii'
    ...
    def __init__(self, data, packet_format='dash'):
        ...
        if packet_format == 'sled':
            ...
            # print('length of formats: ',len(self.sled_format),'|',len(self.dash_format))
            if len(data) == 324:
                newData = bytearray(311)
                # buffer1[pos:pos+len(buffer2)] = buffer2 - if buffer2 is shorter than buffer1
                # buffer2 = 311, buffer1 = 324
                # use slice to get
                # print(type(newData))
                '''
                Array.Copy(Array src, int srcIndex, Array dest, int destIndex, int length)
                Array.Copy(data, 244, newData, 232, 79);
                make a new byte array of 311
                copy data[0] up to data[232]
                '''

                first_chunk = data[0:232] # we need all but the last 79 elements
                arr_first = bytearray(first_chunk)
                print('arr_first:',len(arr_first))

                second_chunk = data[-79:] # take the last 79 elements
                arr_second = bytearray(second_chunk)

                print('arr_second:',len(arr_second))

                arr_first.extend(arr_second)
                newData = arr_first
                #print('size of arrs:',len(newData),len(data))
                data = newData

            for prop_name, prop_value in zip(self.sled_props + self.dash_props, # added together becuse sled_format has all but the final 26 of dash_format
                                             unpack(self.dash_format, data)):
                #print('setting',prop_name,'to',prop_value)
                setattr(self, prop_name, prop_value)
